...
class RangerSparkPlugin private extends RangerBasePlugin("hive", "sparkSql") {
  import RangerSparkPlugin._

  private val LOG = LogFactory.getLog(classOf[RangerSparkPlugin])

  lazy val fsScheme: Array[String] = getConfig()
    .get("ranger.plugin.hive.urlauth.filesystem.schemes", "s3:,hdfs:,file:")
    .split(",")
    .map(_.trim)

  override def init(): Unit = {
    super.init()
    val cacheDir = new File(getConfig().get("ranger.plugin.hive.policy.cache.dir"))
    if (cacheDir.exists() &&
...
//  private val rangerConf: RangerConfiguration = getC
//
//  val showColumnsOption: String = getConfig().get(
//    "xasecure.spark.describetable.showcolumns.authorization.option", "NONE")

  val showColumnsOption = "NONE"
